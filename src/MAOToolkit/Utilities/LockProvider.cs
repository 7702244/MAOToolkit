...
namespace WebApplication.Core.Services
{
    ...
    {
        static readonly LazyConcurrentDictionary<T, InnerSemaphore> LockDictionary = new();
        ...
        public bool Wait(T idToLock, int millisecondsTimeout)
        {
            ...
            {
                if (!semaphore.HasWaiters && LockDictionary.TryRemove(idToLock, out _))
                    semaphore.Dispose();
                return false;
            }
            return true;
        ...
        public void Release(T idToUnlock)
        {
            ...
            {
                semaphore.Release();
                if (!semaphore.HasWaiters && LockDictionary.TryRemove(idToUnlock, out _))
                    semaphore.Dispose();
            }
        }
    }
        ...
        where TKey : notnull
    {
        ...
        public bool TryGetValue(TKey key, out TValue? value)
        {
            ...
            value = success && lazyResult is not null ? lazyResult.Value : default;
            return success;
        }
